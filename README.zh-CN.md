...
Wave AI 是一个可自行部署的 Agent as a Service（Agent 即服务）平台。你定义 Agent，通过 API 提交任务；平台统一管理 Agent 的运行、调度和状态。
...
`make down` 停止容器并保留数据卷。数据库初始化只在空库中创建当前表结构，项目不提供历史迁移；切换版本前请参阅[数据库初始化说明](deploy/sandbox-resources.md#database-initialization--数据库初始化)。
...
默认使用 Docker + gVisor，也支持 rootless Podman 和 sbx。运行沙箱任务前，按[沙箱部署指南](deploy/sandbox-resources.md)准备宿主机。`local` 后端以服务用户的宿主机权限执行命令，仅适用于可信开发环境。
...
| 测量运行时与沙箱性能 | [压测指南](tools/bench/README.zh-CN.md) |
...
日志、任务 Trace、真实模型压测与回归对比，见[观测与压测说明](tools/bench/OBSERVABILITY.zh-CN.md)。
